...
class IsinConverter:
    ...
    def get_symbols(self, isin: str) -> set[str]:
        """Return the symbols associated with the input ISIN."""
    ...
    def get(self, isin: str) -> str:
        """Return a single symbol for the ISIN, preferring shorter tickers."""
        symbols = self.get_symbols(isin)
        if not symbols:
            raise IsinTranslationError(f"No symbol found for ISIN: {isin}")
