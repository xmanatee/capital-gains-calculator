...
def build_transaction(
    row: JsonRowType, file: str, field_names: FieldNames
) -> BrokerTransaction:
    names = field_names
    description = row[names.description]
    raw_action = row[names.action]
    action = parse_schwab_action(raw_action)
    ...
    if row[names.action] == "Deposit":
        ...
        date = datetime.datetime.strptime(details[names.vest_date], "%m/%d/%Y").date()
    ...
    if (
        transaction.date <= datetime.date(2022, 7, 15)
        and transaction.price
        and transaction.price > threshold_price
        and transaction.quantity
    ):
        transaction.price = round_decimal(
            transaction.price / split_factor, ROUND_DIGITS
        )
        transaction.quantity = round_decimal(
            transaction.quantity * split_factor, ROUND_DIGITS
        )
...
def read_schwab_equity_award_json_transactions(
    ...
    with Path(transactions_file).open(encoding="utf-8") as json_file:
        ...
        transactions = [
            build_transaction(transac, transactions_file, fields)
